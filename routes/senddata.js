...
const mailWriter = (value) => {
    let txt = ""
    const content = axios.get("https://www.kontests.net/api/v1/" + value).then((response) => {
...
const sendMsg = async (dat, email_msg) => {
    dat.forEach(async (ele) => {
        if (ele.choices) {
                ...
                switch (choice[i]) {
                    case "Code Chef":
                        if (email_msg.code_chef != "") {
                            await send(ele.email, `CodeChef Contest`, email_msg.code_chef)
                        }
                        break;
                    case "Codeforces":
                        if (email_msg.codeforces != "") {
                            await send(ele.email, `Codeforces Contest`, email_msg.codeforces)
                        }
                        break
                    case "Leet Code":
            ...
            if (email_msg.leet_code != "") {
                await send(ele.email, `LeetCode Contest`, email_msg.leet_code)
            }
        }
    })
}
